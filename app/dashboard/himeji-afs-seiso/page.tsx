...
export default function UniversalDashboardPage() {
  ...
  const formatVal = (val: number, title?: string) => {
    ...
    if (/単価|時給/.test(title)) return Number(val.toFixed(1)).toLocaleString();
  ...
  const handleToggleMetricSetting = async (metricTitle: string, field: 'is_pinned' | 'is_hidden', currentVal: boolean) => {
    try {
      ...
      const payload: any = { location_id: LOCATION_ID, tab_id: activeTab, metric_title: metricTitle, is_pinned: field === 'is_pinned' ? !currentVal : (existing ? existing.is_pinned : false), is_hidden: field === 'is_hidden' ? !currentVal : (existing ? existing.is_hidden : false) };
  ...
  // =========================================================
  // 📊 データ成形ロジック（ベースコード準拠：今期・前期・月フィルター）
  // =========================================================
  ...
  // 💡 ベースコード由来の美しいグリッド表示用成形ロジック（今期・前期対応）
  const getCombinedMetrics = () => {
    ...
    data.salesData.forEach((item: any) => {
      ...
      // 💡 今月・先月・前年 ではなく「今期」「前期」を剥がして共通の名前を作る
  ...
  const filteredDxItems = dxItems.filter(item => {
    ...
    return matchesSearch && matchesTab && matchesHidden;
  });

  // 🌟 自動色分け関数（カテゴリ名から自動で色を決定する）
  const getCategoryColor = (categoryName: string) => {
    const safeCategory = String(categoryName || '未分類').trim();
    const palettes = [
      { hex: '#4f46e5', text: 'text-indigo-600', badgeBg: 'bg-indigo-100', badgeText: 'text-indigo-700' },     // 藍色
      { hex: '#059669', text: 'text-emerald-600', badgeBg: 'bg-emerald-100', badgeText: 'text-emerald-700' }, // 緑色
      { hex: '#ea580c', text: 'text-orange-600', badgeBg: 'bg-orange-100', badgeText: 'text-orange-700' },    // オレンジ
      { hex: '#db2777', text: 'text-pink-600', badgeBg: 'bg-pink-100', badgeText: 'text-pink-700' },          // ピンク
      { hex: '#2563eb', text: 'text-blue-600', badgeBg: 'bg-blue-100', badgeText: 'text-blue-700' },          // 青色
      { hex: '#9333ea', text: 'text-purple-600', badgeBg: 'bg-purple-100', badgeText: 'text-purple-700' },    // 紫色
    ];
    let hash = 0;
    for (let i = 0; i < safeCategory.length; i++) hash = safeCategory.charCodeAt(i) + ((hash << 5) - hash);
  ...
  const handleOpenEditModal = (index: number) => {
    ...
    const item = filteredDxItems[index];
    setNewItem({
      name: item.name || '', effect: item.effect === '未入力' ? '' : (item.effect || ''), category: item.category || '', url: item.url || '',
  ...
  const handleSaveItem = async () => {
    ...
    if (editingIndex !== null) {
      payload.id = filteredDxItems[editingIndex].id; 
  ...
  const handleDeleteItem = async (indexToDelete: number) => {
    await supabaseRequest('dx_actions', 'DELETE', { id: filteredDxItems[indexToDelete].id });
    await fetchSupabaseData();
                  ...
                  return (
                    <div key={index} className={`bg-white border p-6 rounded-[2rem] shadow-sm flex flex-col sm:flex-row gap-6 items-center relative overflow-hidden transition-all hover:shadow-md ${item.is_hidden ? 'opacity-50 bg-slate-50 border-dashed':'border-slate-200'}`}>
                      <div className="absolute top-4 right-4 flex gap-2">
                        ...
                        <button onClick={() => handleOpenEditModal(index)} className="w-8 h-8 flex items-center justify-center rounded-full bg-white border border-slate-200 text-slate-400 shadow-sm hover:text-blue-500 transition-all"><Edit2 size={14} /></button>
                        <button onClick={() => { if(confirm("本当に消去しますか？")) handleDeleteItem(index); }} className="w-8 h-8 flex items-center justify-center rounded-full bg-white border border-slate-200 text-slate-400 shadow-sm hover:text-rose-500 transition-all"><X size={15} /></button>
                      </div>
