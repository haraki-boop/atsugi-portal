...
export default function UniversalDashboardPage() {
  ...
  const getCategoryColor = (categoryName: string) => {
    ...
    const palettes = [
      { hex: '#4f46e5', text: 'text-indigo-600', badgeBg: 'bg-indigo-100', badgeText: 'text-indigo-700' },
      { hex: '#059669', text: 'text-emerald-600', badgeBg: 'bg-emerald-100', badgeText: 'text-emerald-700' },
      { hex: '#ea580c', text: 'text-orange-600', badgeBg: 'bg-orange-100', badgeText: 'text-orange-700' },
      { hex: '#db2777', text: 'text-pink-600', badgeBg: 'bg-pink-100', badgeText: 'text-pink-700' },
      { hex: '#2563eb', text: 'text-blue-600', badgeBg: 'bg-blue-100', badgeText: 'text-blue-700' },
      { hex: '#9333ea', text: 'text-purple-600', badgeBg: 'bg-purple-100', badgeText: 'text-purple-700' },
    ];
    let hash = 0;
  ...
  // 🌟 箒（ほうき）で「さっさっ」と掃き掃除する仕様に生まれ変わったローディング画面！
  if (!data || !isMounted) {
    return (
      <div className="h-screen bg-cyan-50 flex flex-col items-center justify-center relative overflow-hidden notranslate" translate="no">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-cyan-400/20 rounded-full blur-[100px] pointer-events-none animate-pulse" />
        <div className="relative z-10 flex flex-col items-center">
          <div className="bg-white px-6 py-3.5 rounded-2xl mb-8 shadow-sm border border-cyan-100 flex items-center justify-center gap-3">
            {/* 左の箒（さっさっ） */}
            <div className="text-2xl" style={{ animation: 'sweep 1s ease-in-out infinite' }}>🧹</div>
            <img src="/pal-logo.png" alt="PAL Logo" className="h-8 md:h-10 w-auto object-contain" />
            {/* 右の箒（タイミングをずらしてさっさっ） */}
            <div className="text-2xl" style={{ animation: 'sweep 1s ease-in-out infinite 0.5s' }}>🧹</div>
          </div>
          ...
          <div className="w-64 h-1.5 bg-cyan-200/50 rounded-full overflow-hidden mb-6 shadow-inner relative">
            <div className="absolute top-0 left-0 h-full bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full w-1/2" style={{ animation: 'loading-slide 2s ease-in-out infinite' }} />
          </div>
          <div className="flex items-center gap-3 text-cyan-700">
            <Loader2 className="animate-spin text-cyan-500" size={18} />
            <span className="text-[11px] font-bold tracking-widest uppercase">Fetching & Sweeping Data...</span>
          </div>
        </div>
        {/* 箒が「さっさっ」と動くアニメーションを追加 */}
        <style dangerouslySetInnerHTML={{__html: `
          @keyframes loading-slide { 0% { transform: translateX(-100%); width: 50%; } 100% { transform: translateX(250%); width: 50%; } }
          @keyframes sweep { 
            0%, 100% { transform: rotate(15deg) translateY(0); } 
            50% { transform: rotate(-25deg) translateX(-8px) translateY(4px); } 
          }
        `}} />
      </div>
    );
