...
export default function CompareDashboardPage() {
  ...
  const fetchSupabaseAllData = async () => {
    try {
      ...
      const headers: any = { 'apikey': token, 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' };
      const baseUrl = 'https://ukhcalayaazwmufewsks.supabase.co/rest/v1';

      const [dxRes, envRes, histRes] = await Promise.all([
        fetch(`${baseUrl}/dx_actions?order=id.asc`, { headers }),
        fetch(`${baseUrl}/env_actions?order=id.asc`, { headers }),
        fetch(`${baseUrl}/sales_history?order=id.asc`, { headers })
      ]);
