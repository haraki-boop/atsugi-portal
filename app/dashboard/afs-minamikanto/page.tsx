...
export default function AfsMinamikantoDashboardPage() {
  ...
  // 🚀 【南関東カスタム】工数分析タブの項目を「畜産」と「水産」に変更
  const generateStackedManhoursData = () => {
    ...
    const col_Chikusan = logisticsItems.find(item => item && item.title && (item.title === "畜産工数" || item.title === "実績_畜産工数"));
    const col_Suisan = logisticsItems.find(item => item && item.title && (item.title === "水産工数" || item.title === "実績_水産工数"));

    return currentMonthIndices.map(idx => {
      const label = baseLabels[idx];
      const totalH = colV_Total && colV_Total.values ? n(colV_Total.values[idx]) : 0;
      const chikusanH = col_Chikusan && col_Chikusan.values ? n(col_Chikusan.values[idx]) : 0;
      const suisanH = col_Suisan && col_Suisan.values ? n(col_Suisan.values[idx]) : 0;
            ...
            <div className="h-[450px] bg-slate-50/50 p-6 rounded-3xl border border-slate-100 min-w-0">
              <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                <ComposedChart data={generateStackedManhoursData()} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  {/* 🚀 【南関東カスタム】積み上げ棒グラフの項目と色を「畜産」と「水産」に変更 */}
                  <Bar name="畜産" dataKey="畜産" stackId="afsManpower" fill="#f43f5e" />
                  <Bar name="水産" dataKey="水産" stackId="afsManpower" fill="#0ea5e9" />
